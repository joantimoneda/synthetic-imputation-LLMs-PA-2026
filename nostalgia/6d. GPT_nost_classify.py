...
all_data = pd.concat([data, response], axis=1)
all_data

all_data.to_excel("GPT_nost_annotations.xlsx")
...
overall_f1 = f1_score(data["nostalgic"], response, average="weighted") # 0.885
print("Overall F1:", overall_f1)

f1_score(data["nostalgic"], response, labels=[0], average="weighted") # 0.798
f1_score(data["nostalgic"], response, labels=[1], average="weighted") # 0.915
