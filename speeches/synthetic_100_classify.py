...
synth_new["label"] = 1
synth_new = synth_new.sample(n=118).reset_index(drop=True)

other_cats = not_intl.drop(["text", "preds"], axis=1)
other_cats.rename(columns={"text_eng": "text"}, inplace=True)
other_cats["synthetic"] = 0

real100 = pd.read_excel("../../data samples/intl_speeches_sample_100.xlsx", index_col=0) # use 
...
all_synth.synthetic.value_counts()

all_synth.to_excel("GPT_synthetic_labeled_100_118.xlsx")
