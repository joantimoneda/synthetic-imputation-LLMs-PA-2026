...
## Data: 
import pandas as pd
data = pd.read_excel("../../../speeches_intl/data samples/intl_speeches_sample_50.xlsx", index_col=0)
...
with open('txt_50_speeches.txt', 'w') as f:
    for entry in text:
        f.write(entry + " \n ")
...
parser.add_argument('-i', '--in-file', type=str, default='txt_50_speeches.txt',
...
parser.add_argument('-o', '--output-prefix', type=str, default='out_50_speeches',)
...
synthetic = pd.Series([0]*50 + [1]*168)
...
combined.to_excel("ssmba_50_speeches.xlsx")
...
combined_final = pd.concat([combined, not_intl], axis=0)
...
combined_final.to_excel("ssmba_50_full_speeches.xlsx")
