...
## Data: 
import pandas as pd
data = pd.read_excel("../../../speeches_intl/data samples/intl_speeches_sample_100.xlsx", index_col=0)

text = data.loc[:,"text_eng"]
text = [x.strip() for x in text]
text = [x+"\n" for x in text]
text = [x.replace("|", "") for x in text]
len(text)

with open('txt_100_speeches.txt', 'w') as f:
    for entry in text:
        f.write(entry + " \n ")
...
parser.add_argument('-m', '--model', type=str, default='roberta-large',
...
parser.add_argument('-i', '--in-file', type=str, default='txt_100_speeches.txt',
        help='txt_example.txt')
...
parser.add_argument('-o', '--output-prefix', type=str, default='out_100_speeches',)

parser.add_argument('-p', '--noise-prob', type=float, default=0.40,
...
with open('out_100_speeches', 'r') as f:
        output = f.readlines()
...
combined = text + subset
len(combined)

label = pd.Series([2]*len(combined))
speech = pd.Series(["international"]*len(combined))

synthetic = pd.Series([0]*100 + [1]*118)

combined = pd.concat([speech.rename("speechtype"), pd.Series(combined).rename("text_eng"), label.rename("label"), synthetic.rename("synthetic")], axis=1)

combined.to_excel("ssmba_100_speeches.xlsx")


all_data = pd.read_excel("../../../speeches_intl/data raw/speeches_and_scores_label_ENG.xlsx", index_col=0)
all_data

all_data = all_data.loc[:,["speechtype", "label", "text_eng"]]

not_intl = all_data[all_data.speechtype!="international"].reset_index(drop=True)
not_intl["synthetic"] = [0]*len(not_intl)
not_intl

combined_final = pd.concat([combined, not_intl], axis=0).reset_index(drop=True)
combined_final = combined_final.sample(frac=1).reset_index(drop=True)
combined_final 
combined_final.speechtype.value_counts()
combined_final.synthetic.value_counts()

combined_final.to_excel("ssmba_100_full_speeches.xlsx")
