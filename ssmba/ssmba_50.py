...
## Data: 
import pandas as pd
data = pd.read_excel("../../nostalgia/data samples/fake_nost_50.xlsx", index_col=0)

text = data.loc[:,"text"]
text = [x+"\n" for x in text]

with open('txt_50_nost.txt', 'w') as f:
    for entry in text:
        f.write(entry + " \n ")
...
parser.add_argument('-i', '--in-file', type=str, default='txt_50_nost.txt',
...
parser.add_argument('-o', '--output-prefix', type=str, default='out_50',)
...
combined = text + first + second + [last_one]
len(combined)

nostalgic = pd.Series([1]*151)

synthetic = pd.Series([0]*50 + [1]*101)

combined = pd.concat([nostalgic.rename("nostalgic"), pd.Series(combined).rename("text"), synthetic.rename("synthetic")], axis=1)

combined.to_excel("ssmba_50_101.xlsx")

all_data = pd.read_excel("../../nostalgia/nostalgia_data_clean_sample.xlsx", index_col=0)
all_data

not_nost = all_data[all_data.nostalgic==0].reset_index(drop=True)
not_nost = not_nost.drop("train", axis=1)
not_nost["synthetic"] = [0]*len(not_nost)
not_nost

combined_final = pd.concat([combined, not_nost], axis=0).reset_index(drop=True)
combined_final = combined_final.sample(frac=1)
combined_final = combined_final[combined_final.text!="#NAME?"].reset_index(drop=True)
combined_final 
combined_final.nostalgic.value_counts()
combined_final.synthetic.value_counts()


combined_final.to_excel("ssmba_50_101_full.xlsx")
